...
import {sortProducts} from '~/utilities/WPHelpers';
const WPShopProducts = (props) => {
    ...
    function handleSortChange(event) {
        ...
        handlePagination(1, 12, selectedOption);
    }
    useEffect(() => {
        if(sortingOption === 'price_asc') {
            dispatch(WPGetProducts({ orderby: 'price' , order: 'asc'}));
        }else
        dispatch(WPGetProducts({ orderby: sortingOption }));
    }, [sortingOption]);
    ...
    if (!WPLoading) {
        if (WPProducts && WPProducts.items) {
            sortingOption?WPProducts.items = sortProducts(WPProducts.items, sortingOption):WPProducts.items;
            countProductsView = (
                <p>
...
export default connect()(WPShopProducts);
